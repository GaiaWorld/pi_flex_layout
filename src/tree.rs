...
pub struct Layout<'a, K: Null + Clone + Copy, S, T, L, I, R, LI: Get<K, Target = L>, LR: LayoutR>(pub LayoutContext<'a, K, S, T, L, I, R, LI, LR>);
...
where 
	...
	pub fn compute(&mut self, dirty: &mut LayerDirty<K>) {
		...
		for (id, _layer) in dirty.iter() {
			...
			let (child_head, child_tail) = self.0.tree.get_down(*id).map_or((K::null(), K::null()), |down|{(down.head, down.tail)});
			...
			if state.abs() {
				...
				while !parent.is_null() && self.0.i_nodes[parent].state.vnode() {
					parent = self.0.tree.get_up(parent).map_or(K::null(), |up|{up.parent()});
				}; 
	...
	fn set_self_dirty(dirty: &mut LayerDirty<K>, id: K, parent: K, layer: usize, i_node: &mut INode) -> K {
		debug_println!("set_self_dirty, id: {}, self_dirty:{}, children_dirty:{:?}, layer:{}", id, i_node.state.self_dirty(), i_node.state.children_dirty(), self.0.tree.get_layer(id));
