...
impl Default for CharNode {
	fn default() -> Self {
		CharNode {
			...
			size: Size{width: Dimension::Points(0.0), height: Dimension::Points(0.0)},
			pos: Rect {
				top: 0.0,
				right: 0.0,
				bottom: 0.0,
...
impl<K: Null + Clone> Cache<K> {
    ...
    fn text_layout(
        ...
        while char_index < len {
            ...
            let (main_d, cross_d) = self.temp.main_cross(match r.size.width {
...
/// 布局上下文
pub struct LayoutContext<'a, K: Null + Clone + Copy, S, T, L, I, R, LI: Get<K, Target = L>, LR: LayoutR> {
...
where 
	...
	fn layout_node(
		...
		match temp {
			TempType::CharIndex(r) => {
				...
				out_any!(debug_println,"set_layout text: {:?}, {:?}", Rect {
					left: width.0,
					top: height.0,
					right: width.0 + width.1, // TODO
					bottom: height.0 + height.1,
    ...
    fn node_layout(
        ...
        while !child.is_null() {
            let (next, prev) = self.tree.get_up(child).map_or((K::null(), K::null()), |up|{(up.next(), up.prev())});
			let (child_head, child_tail) = self.tree.get_down(child).map_or((K::null(), K::null()), |down|{(down.head(), down.tail())});
            ...
            let (main_d, cross_d) = cache
                .temp
				.main_cross(style.width(), style.height());
			...
			let (max_main, max_cross) = cache
                .temp
				.main_cross(max_width, max_height);
			let (min_main, min_cross) = cache
                .temp
            ...
            let mut info = RelNodeInfo {
				...
				cross: min_max_calc(cross_d.resolve_value(cache.cross_value),min_cross,max_cross),
                margin_main: 0.0,
                margin_main_start: calc_number((margin.0).0, cache.main_value),
                margin_main_end: calc_number((margin.0).1, cache.main_value),
                margin_cross_start: calc_number((margin.1).0, cache.cross_value),
                margin_cross_end: calc_number((margin.1).1, cache.cross_value),
            ...
            let temp = if w == Number::Undefined || h == Number::Undefined {
               ...
               out_any!(debug_println, 
                ...
                let mut cache_new = Cache::new(
					...
					out_any!(debug_println, "cache, main_line: {:?}, id: {:?}", cache_new.main_line, id);
				// cache.main_line = 
	...
	fn set_layout(
		...
		{
			set_layout_result(&mut layout, self.notify, self.notify_arg, id, pos, size, &border, &padding)
		} else {
			LayoutSize::None
		};
		// 递归布局子节点
...
impl<T: FlexLayoutStyle> FlexLayoutCombine for T  {
	...
	fn calc_horizontal_content_size(&self, size: Number) -> Number {
		calc_content_size(size, self.border_left(), self.border_right(), self.padding_right(), self.padding_right())
	}

	fn calc_vertical_content_size(&self, size: Number) -> Number {
		calc_content_size(size, self.border_top(), self.border_bottom(), self.padding_top(), self.padding_bottom())
	}
}
