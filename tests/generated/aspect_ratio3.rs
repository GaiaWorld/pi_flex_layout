...
fn aspect_ratio3() {
    ...
    layout_tree.insert(
        ...
        pi_flex_layout::prelude::Style {
            flex_shrink: 1f32,
            flex_basis: pi_flex_layout::prelude::Dimension::Points(210f32),
    ...
    assert_eq!(layout.rect.left.round(), 0f32);
    assert_eq!(layout.rect.top.round(), 48f32);
    let layout = layout_tree.get_layout(node_4).unwrap();
    assert_eq!((layout.rect.right - layout.rect.left).round(), 200f32);
    assert_eq!((layout.rect.bottom - layout.rect.top).round(), 95f32);
    assert_eq!(layout.rect.left.round(), 0f32);
    assert_eq!(layout.rect.top.round(), 257f32);
}
