...
fn flex_grow_less_than_factor_one() {
    ...
    layout_tree.insert(
        ...
        pi_flex_layout::prelude::Style {
            flex_grow: 0.2f32,
            flex_shrink: 0f32,
            flex_basis: pi_flex_layout::prelude::Dimension::Points(40f32),
            ..Default::default()
        },
    );
    ...
    assert_eq!((layout.rect.right - layout.rect.left).round(), 132f32);
    assert_eq!((layout.rect.bottom - layout.rect.top).round(), 0f32);
    assert_eq!(layout.rect.left.round(), 0f32);
    assert_eq!(layout.rect.top.round(), 0f32);
    let layout = layout_tree.get_layout(node_4).unwrap();
    assert_eq!((layout.rect.right - layout.rect.left).round(), 92f32);
    assert_eq!((layout.rect.bottom - layout.rect.top).round(), 0f32);
    assert_eq!(layout.rect.left.round(), 132f32);
    assert_eq!(layout.rect.top.round(), 0f32);
    let layout = layout_tree.get_layout(node_5).unwrap();
    assert_eq!((layout.rect.right - layout.rect.left).round(), 184f32);
    assert_eq!((layout.rect.bottom - layout.rect.top).round(), 0f32);
    assert_eq!(layout.rect.left.round(), 224f32);
    assert_eq!(layout.rect.top.round(), 0f32);
}
