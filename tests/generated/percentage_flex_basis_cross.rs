...
fn percentage_flex_basis_cross() {
    ...
    layout_tree.insert(
        ...
        pi_flex_layout::prelude::Style {
            flex_grow: 1f32,
            flex_basis: pi_flex_layout::prelude::Dimension::Percent(0.25f32),
            ..Default::default()
        },
    );
    ...
    assert_eq!((layout.rect.bottom - layout.rect.top).round(), 250f32);
    assert_eq!(layout.rect.left.round(), 0f32);
    assert_eq!(layout.rect.top.round(), 0f32);
    let layout = layout_tree.get_layout(node_4).unwrap();
    assert_eq!((layout.rect.right - layout.rect.left).round(), 0f32);
    assert_eq!((layout.rect.bottom - layout.rect.top).round(), 150f32);
    assert_eq!(layout.rect.left.round(), 0f32);
    assert_eq!(layout.rect.top.round(), 250f32);
}
